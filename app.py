...
def process_single_file(input_file, portfolio_name):
    ...
    for idx, row in df_input.iterrows():
        ...
        # ✅ NEW: Only add row if it passes all criteria
        # Check if ISIN is valid AND Cost Price is not empty AND Quantity is not 0
        if isin_value != "" and cost_price != "" and quantity != 0:
            output_row = {
...
def filter_dataframe(df):
    """Filter dataframe to remove rows with blank ISIN, blank Cost Price, or 0 Position"""
    ...
    # Count rows that will be removed for each condition
    blank_isin_count = len(filtered_df[filtered_df['ISIN'] == ""])
    blank_cost_count = len(filtered_df[filtered_df['Cost Price'] == ""])
    zero_position_count = len(filtered_df[filtered_df['Position'] == 0])
    
    # Apply filters
    filtered_df = filtered_df[filtered_df['ISIN'] != ""]  # Remove blank ISIN
    filtered_df = filtered_df[filtered_df['Cost Price'] != ""]  # Remove blank Cost Price
    filtered_df = filtered_df[filtered_df['Position'] != 0]  # Remove 0 Position
    
    # Count rows removed (some rows may fail multiple conditions)
    ...
    return filtered_df, {
        ...
        'blank_isin': blank_isin_count,
        'blank_cost': blank_cost_count,
        'zero_position': zero_position_count
    }
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🔍 **FILTERING RULES**", expanded=True):
            st.markdown("""
            **Rows are automatically removed if:**
            
            • ❌ **ISIN is blank/empty**
            • ❌ **Cost Price is blank/empty**
            • ❌ **Position (Quantity) is 0**
            
            **Result:**
            ✅ Only rows with valid ISIN, non-empty Cost Price, and non-zero Quantity remain
            """)
        ...
        if 'filter_stats' in st.session_state:
            with st.expander("🔍 **FILTERING DETAILS**", expanded=False):
                stats = st.session_state.filter_stats
                st.markdown(f"""
                **Filtering Applied:**
                • Original rows: **{stats['original']:,}**
                • After filtering: **{stats['final']:,}**
                • Rows removed: **{stats['removed']:,}**
                
                **Removed because:**
                • Blank ISIN: **{stats['blank_isin']:,}**
                • Blank Cost Price: **{stats['blank_cost']:,}**
                • Zero Quantity: **{stats['zero_position']:,}**
                """)
    ...
    if uploaded_files:
            ...
            with st.spinner("Processing your files..."):
                ...
                if result:
                    ...
                    if processed_files > 0:
                        ...
                        df_filtered, filter_stats = filter_dataframe(df_combined)
                        
                        # Store in session state for sidebar
                        st.session_state.processed_files = processed_files
                        st.session_state.total_rows = len(df_filtered)
                        st.session_state.valid_isins = (df_filtered['ISIN'] != "").sum()
                        st.session_state.filter_stats = filter_stats  # Store filter stats
                        
                        # Success message with filter info
                        st.markdown(f"""
                        <div class="success-box">
                        ✅ Successfully processed {processed_files} file(s)<br>
                        🔍 Filtered: {filter_stats['removed']} rows removed 
                        (Blank ISIN: {filter_stats['blank_isin']}, 
                        Blank Cost: {filter_stats['blank_cost']}, 
                        Zero Qty: {filter_stats['zero_position']})
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        display_data_preview(df_filtered)
                        ...
                        with stat_col2:
                            valid_isins = (df_filtered['ISIN'] != "").sum()
                            st.metric("Valid ISINs", f"{valid_isins:,}")
                        with stat_col3:
                            unique_portfolios = df_filtered['Portfolio Name'].nunique()
                        ...
                        if unique_portfolios > 1:
                            ...
                            portfolio_counts = df_filtered['Portfolio Name'].value_counts()
                            for portfolio, count in portfolio_counts.items():
                                st.progress(min(count / len(df_filtered), 1.0), 
                                          text=f"{portfolio}: {count} rows ({count/len(df_filtered)*100:.1f}%)")
                        ...
                        try:
                            ...
                            portfolio_count = df_filtered['Portfolio Name'].nunique()
                            ...
                            with col2:
                                st.download_button(
                                    ...
                                    help=f"Download {len(df_filtered):,} rows from {processed_files} files"
                                )
                            ...
                            • Column Widths: Auto-adjusted for readability<br>
                            • Filtered: Blank ISIN, blank Cost Price, and zero Quantity rows removed
                            </div>
                            """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            • Blank ISIN, blank Cost Price, and zero Quantity rows are automatically removed
            </div>
            """, unsafe_allow_html=True)
